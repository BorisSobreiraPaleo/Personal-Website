...
const DateItem = ({year, title, subtitle, details, techs}) => {
  return (
    <ol className="flex flex-col md:flex-row relative border-l border-[#03396c]" >
      <li className="mb-10 ml-4">
        ...
        <p className="flex flex-wrap gap-4 flex-row items-center justify-start text-xs md:text-sm">
          <div className="flex flex-wrap items-baseline justify-left [&>*]:m-1 dark:text-[#F4F4F9] text-[#000B11]">
          ...
          <span className="table-cell my-1 text-sm font-normal leading-none text-[#60a5fa]">{subtitle}</span>
            {
              techs.map((tech)=>(
                <div className='mx-1' key={tech}>{tech}</div>
              ))
            }
          </div>
        </p>
